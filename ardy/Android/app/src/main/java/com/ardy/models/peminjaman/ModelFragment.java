...
public class ModelFragment extends Fragment {
    ...
    public void showCreateModal() {
        ...
        // TODO EDIT
        EditText tanggalPinjam = Utils.createEditText(context, "tanggalPinjam");
        inputContainer.addView(tanggalPinjam);
        EditText tanggalKembali = Utils.createEditText(context, "tanggalKembali");
        inputContainer.addView(tanggalKembali);
        EditText tanggalJatuhTempo = Utils.createEditText(context, "tanggalJatuhTempo");
        inputContainer.addView(tanggalJatuhTempo);
        EditText status = Utils.createEditText(context, "status");
        inputContainer.addView(status);
        EditText denda = Utils.createEditText(context, "denda");
        inputContainer.addView(denda);
        EditText bukuId = Utils.createEditText(context, "bukuId");
        inputContainer.addView(bukuId);
        ...
        dialogView.findViewById(R.id.createButton).setOnClickListener(v -> {
            ...
            Model createdModel = new Model(
                    tanggalPinjam.getText().toString(),
                    tanggalKembali.getText().toString(),
                    tanggalJatuhTempo.getText().toString(),
                    status.getText().toString(),
                    Utils.tryParseFloat(denda.getText().toString(), 0),
                    Utils.tryParseInt(bukuId.getText().toString(), 0)
            );
    ...
    public void showUpdateModal(Integer id, Model model) {
        ...
        // TODO EDIT
        EditText tanggalPinjam = Utils.createEditText(context, "tanggalPinjam");
        tanggalPinjam.setText(model.tanggalPinjam);
        inputContainer.addView(tanggalPinjam);

        EditText tanggalKembali = Utils.createEditText(context, "tanggalKembali");
        tanggalKembali.setText(model.tanggalKembali);
        inputContainer.addView(tanggalKembali);

        EditText tanggalJatuhTempo = Utils.createEditText(context, "tanggalJatuhTempo");
        tanggalJatuhTempo.setText(model.tanggalJatuhTempo);
        inputContainer.addView(tanggalJatuhTempo);
        ...
        EditText denda = Utils.createEditText(context, "denda");
        denda.setText(String.valueOf(model.denda));
        inputContainer.addView(denda);

        EditText bukuId = Utils.createEditText(context, "bukuId");
        bukuId.setText(String.valueOf(model.bukuId));
        inputContainer.addView(bukuId);
        //
        ...
        updateButton.setOnClickListener(v -> {
            ...
            if (!Objects.equals(model.tanggalPinjam, tanggalPinjam.getText().toString())) {
                updatedModel.put("tanggalPinjam", tanggalPinjam.getText().toString());
            }
            if (!Objects.equals(model.tanggalKembali, tanggalKembali.getText().toString())) {
                updatedModel.put("tanggalKembali", tanggalKembali.getText().toString());
            }
            if (!Objects.equals(model.tanggalJatuhTempo, tanggalJatuhTempo.getText().toString())) {
                updatedModel.put("tanggalJatuhTempo", tanggalJatuhTempo.getText().toString());
            }
            if (!Objects.equals(model.status, status.getText().toString())) {
                updatedModel.put("status", status.getText().toString());
            }
            if (!Objects.equals(model.denda, denda.getText().toString())) {
                updatedModel.put("denda", denda.getText().toString());
            }
            if (!Objects.equals(model.bukuId, bukuId.getText().toString())) {
                updatedModel.put("bukuId", bukuId.getText().toString());
            }
            //
